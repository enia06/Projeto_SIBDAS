<?php
// Configurações globais da aplicação
define('APP_NAME', 'Stay This.Positive');
define('APP_VERSION', '1.0.0');
define('APP_COPYRIGHT', '© 2026 Stay This.Positive');
define('BASE_URL', '/sibdas/1241327/stay-this-positive');


define('MYSQL_HOST', 'vsgate-s1.dei.isep.ipp.pt');
define('MYSQL_PORT', '10464');
define('MYSQL_DATABASE', 'db1241327');
define('MYSQL_USERNAME', '1241327');
define('MYSQL_PASSWORD', 'torres_327');
define('MYSQL_AES_KEY', 'SIBDAS_AES_KEY');

define('OPENSSL_METHOD', 'AES-256-CBC');
define('OPENSSL_KEY', 'H0SDRQzIGqclX2kbYBk9xspdn9U5f3Wa');
define('OPENSSL_IV', 'BzKAbjuREsHgnw56');